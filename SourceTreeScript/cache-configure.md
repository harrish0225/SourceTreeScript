...
## {{ai0}} Configure Redis cache settings
...
#### {{ai4}} Maxmemory-policy and maxmemory-reserved
...
#### {{ai5}} Keyspace notifications (advanced settings)
...
## <a {{ri_0b}} name="recommendations"></a> {{ri_e}} Redis Cache Advisor
The {{ri_1b}} **Redis Cache Advisor** {{ri_e}} blade displays recommendations for your cache. During normal operations, no recommendations are displayed. 
...
## {{ai9}} Default Redis server configuration
...
## {{ai10}} Redis commands not supported in Azure Redis Cache
> [!IMPORTANT]
...
{{ri_4b}} > {{ri_e}} 
{{ri_5b}} > {{ri_e}} * BGREWRITEAOF
{{ri_6b}} > {{ri_e}} * BGSAVE
{{ri_7b}} > {{ri_e}} * CONFIG
{{ri_8b}} > {{ri_e}} * DEBUG
{{ri_9b}} > {{ri_e}} * MIGRATE
{{ri_10b}} > {{ri_e}} * SAVE
{{ri_11b}} > {{ri_e}} * SHUTDOWN
{{ri_12b}} > {{ri_e}} * SLAVEOF
{{ri_13b}} > {{ri_e}} * CLUSTER - Cluster write commands are disabled, but read-only Cluster commands are permitted.
> 
> 
