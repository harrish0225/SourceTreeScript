...
from .compare_common import add_back_empty_leadings
...
def get_diff_set(lastmonth_lines, mc_lines, lastmonth_empty_leadings, mc_empty_leadings):
    differ = Differ()
...
def append_one_line_to_diff_set(result, diff_set, types, indices, i):
    diff_set.append([result[i]])
    types.append(result[i][0])
    indices.append(i)
    return

def initiate_diff_set(result):
    i = 1
    diff_set = []
    types = []
    indices = []
    if result[0][0]!=" ":
        append_one_line_to_diff_set(result, diff_set, types, indices, 0)
    while i<len(result):
        if result[i][0]!=" ":
            if result[i][0]!=result[i-1][0]:
                append_one_line_to_diff_set(result, diff_set, types, indices, i)
            else:
                diff_set[len(diff_set)-1].append(result[i])
        i+=1
    return diff_set, types, indices

def compare_result_split2(result):
    diff_set, types, indices = initiate_diff_set(result)
    i=0
    diff_set2 = []
    types2 = []
